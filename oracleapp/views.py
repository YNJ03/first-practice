...
### models.py에 생성한 Cart, Prod 클래스 불러들이기
from .models import Cart, Prod, Lprod, CartMember, CartMemberProd

# - get : 조회할 때 함수이름 사용
# - set : 수정, 삭제할 때 함수이름 사용
...
def setProductInsert(request):   
    ...
    msg = """
        <script type='text/javascript'>
            alert('정상적으로 입력되었습니다!!');
            location.href='/oracle/product_list/';
        </script>
    """   
    return HttpResponse(msg)
...
def getCartMemProdList(request) :
    cart_list = CartMemberProd.objects.all().order_by("-cart_no", "cart_member")
    return render(request, "oracleapp/cartmem/cart_mem_prod_list.html", {"cart_list" : cart_list})
...
def getCartMemList(request) :
    ...
    return render(request, "oracleapp/cartmem/cart_mem_list.html", {"cart_list" : cart_list})
...
def setLprodUpdate(request) :
    ...
    try :     
        Lprod.objects.filter(lprod_gu = lprod_gu).update(lprod_id = lprod_id, lprod_nm = lprod_nm)
    except :
        ### 오류처리
        msg = """
                <script type='text/javascript'>
    ...
    ### 정상처리
    # - history.go(-2)하면 수정된 값이 업데이트가 안되기 때문에 하면 안됨
    msg = """
                <script type='text/javascript'>
                    alert('정상적으로 수정되었습니다!!');
...
def getLprodUpdateView(request) :
    ...
    return render(request, "oracleapp/lprod/lprod_update_view.html", {"lprod_gu" : lprod_gu, "lprod_view" : lprod_view})
...
def setLprodInsert(request) :
    try :     
        ...
        Lprod.objects.create(lprod_gu = lprod_gu, lprod_id = lprod_id, lprod_nm = lprod_nm)
...
def getLprodInsertView(request) :
    return render(request,"oracleapp/lprod/lprod_insert_view.html", {})
...
def getLprodView(request) :
    ...
    return render(request, "oracleapp/lprod/lprod_view.html", {"lprod_gu" : lprod_gu, "lprod_view" : lprod_view})
...
def getLprodList(request) :
    ...
    return render(request, "oracleapp/lprod/lprod_list.html", {"lprod_list" : lprod_list})
...
def setProdUpdate(request) :
    ...
    try :     
        Prod.objects.filter(prod_id = prod_id).update(prod_name = prod_name, prod_cost = prod_cost,
                                                      prod_price = prod_price, prod_sale = prod_sale)
    except :
        ### 오류처리
        msg = """
                <script type='text/javascript'>
...
def getProdUpdateView(request) :
    ...
    return render(request, "oracleapp/prod/prod_update_view.html", {"prod_id" : prod_id, "prod_view" : prod_view})
...
def getProdView(request) :
    ...
    return render(request, "oracleapp/prod/prod_view.html", {"prod_id" : prod_id, "prod_view" : prod_view})
...
def getProdList(request) :
    ...
    return render(request, "oracleapp/prod/prod_list.html", {"prod_list" : prod_list})
...
def setCartInsert(request) :
    ...
    try :     
        Cart.objects.create(cart_no = cart_no, cart_member = cart_member, cart_prod = cart_prod, cart_qty = cart_qty)
    except :
        ### 오류처리
        msg = """
                <script type='text/javascript'>
...
def getCartInsertView(request) :
    return render(request,"oracleapp/cart/cart_insert_view.html", {})
...
def getCartUpdateView(request) :
    ...
    return render(request, "oracleapp/cart/cart_update_view.html", {"cart_no" : cart_no, "cart_prod" : cart_prod, "cart_view" : cart_view})
...
def getCartView(request) :
    ...
    return render(request, "oracleapp/cart/cart_view.html", {"cart_view" : cart_view, "cart_no" : cart_no, "cart_prod" : cart_prod})
...
############## [회원관리] #################
### 회원정보 수정하기
# - 수정에 대한 처리는 Database 처리 후 상세보기 페이지로 이동만 하면 됨
...
def setMemberUpdate(request) :
    ...
    try :
        ...
        ###(해석) member 테이블의 데이터 중에 아이디가 mem_id변수인 것에 대한 mem_pass 컬럼의 값을 mem_pass변수의 값으로 수정,
        #         mem_add1 컬럼의 값을 mem_add1변수의 값으로 수정
        
        Member.objects.filter(mem_id = mem_id).update(mem_pass = mem_pass, mem_add1 = mem_add1)
    except :
        ### 오류처리
        msg = """
                <script type='text/javascript'>
...
def getMemberUpdateView(request) :
    ...
    return render(request, "oracleapp/member/mem_update_view.html", {"mem_id" : mem_id, "mem_view" : mem_view})
...
def getMemberView(request) :
    ...
    return render(request, "oracleapp/member/mem_view.html", {"mem_id" : mem_id, "mem_view" : mem_view})
...
def getMemberList(request) :
    ...
    return render(request, "oracleapp/member/mem_list.html", {"mem_list" : mem_list})
