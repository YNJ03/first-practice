...
# - django라는 폴더에 http라는 파일에 HttpResponse라는 클래스가 있음
...
def getTemplate01(request) :
    mem_name = "이순신"
    mem_add = "부산 수영구~~"
    return render(request, "firstapp/template01.html", 
                  ### front 프로그램 영역으로 데이터를 전달하는 곳
                  # - key는 문자열로 보통 정의
                  # - value는 변수 또는 직접 값을 넣어줘도 됨(모든 형태 넣을 수 있음)
                  {"mem_name" : mem_name, "mem_add" : mem_add})
...
def getIndex1(request) :
    return HttpResponse("<b>firstapp/urls.py : index1 페이지 입니다.</b>")
